...
mfsr_spawn <- spawn_files[c(1:3,5)] |> # leave out 2004 (see below)
  map_df(read_xls, .id = "file", sheet = "AllRch", range = cell_cols(c(1:9)))  |> # load in data, keep cols 1-9
  select(-"file")

# bring in 2004 data separately because it wants to load the easting as a character
data_2004 <- read_xls("./data/russ_spawn/raw/04_ReddMonitor_AvgXY.xls", range = cell_cols(c(1:9)), sheet = "AllRch")
data_2004$EASTING <- as.numeric(data_2004$EASTING) # change easting to a character
