...
```{r map, out.width = "75%", fig.cap = "Map of the Middle Fork Salmon River watershed showing major tributaries and surveyed Chinook Salmon redd locations from 2002 to 2005."}
...
For thermal metrics, we used modeled daily mean stream temperatures from Siegel et al. (2023), downscaled to the COMID level. These modeled temperatures were validated against empirical logger data collected at a subset of sites in the Middle Fork basin, with strong agreement (R² > 0.9).
...
Stream discharge data were obtained from a single gage low in the watershed (USGS Gage 13309220, Middle Fork at Middle Fork Lodge). As with temperature, discharge metrics were linked to individual redd records based on COMID and spawn date.

During exploratory analysis, we found that spawn timing was most strongly associated with temperature metrics—particularly 90-day antecedent temperature (`temp_90_before`)—while flow, post-spawn, and fixed-date metrics exhibited weak or inconsistent relationships. These latter covariates were excluded from further analysis.
...
### Spawn data descriptive statistics and vizualizations
...
Figure \@ref(fig:plot-temp-90) shows the relationship between `temp_90` and spawn date by stream and year.

```{r plot-temp-90, fig.width=8, fig.height=5, fig.cap = "Relationship between `temp_90` and `yday` (spawn date) by stream and year. Solid lines are linear fits."}
...
The model with a quadratic effect of `temp_90` (`m.t90.3`) improved dramatically over the null (intercept-only) model and linear effect (Table \@ref(tab:t90-aic)), and boosting both conditional and marginal R² (Table \@ref(tab:t90-perf)). This confirms a strong, nonlinear effect of pre-spawn temperature on spawn timing (Figure \@ref(fig:t90-relate)).

```{r t90-aic}
compare_aic(m.t90.1, m.t90.2, m.t90.3) |> 
  kable(caption = "AIC comparison of linear mixed models relating spawn date (`yday`) to `temp_90`.")
```

```{r t90-perf}
compare_performance(m.t90.1, m.t90.2, m.t90.3, rank = TRUE, metrics = "common") |> 
  kable(caption = "Model performance of linear mixed models relating spawn date (`yday`) to `temp_90`.")
```

```{r t90-relate, fig.width=8, fig.height=3, fig.cap="Predicted marginal effect of `temp_90` on `yday` (spawn date) for (A) `m.t90.2` and (B) `m.t90.3`."}
...
Table \@ref(tab:t90-aic2) shows that adding stream (`m.t90.4`) or year (`m.t90.5`) individually improved model fit relative to `m.t90.3`, but year alone (`m.t90.5`) performed better than stream alone (`m.t90.4`).

Combining stream and year additively (`m.t90.6`) provided further improvement in AIC, along with the best marginal R2 and RMSE (Table \@ref(tab:t90-perf2)). This suggests that both stream and year provide meaningful structure in explaining variation in spawn timing.
...
```{r t90-relate2, fig.width=5, fig.height=3, fig.cap="Predicted marginal effect (`m.t90.8`) of `temp_90` on `yday` (accounting for the effects of stream and year)."}
...
```{r elev-plots, fig.width=8, fig.height=6, fig.cap = "Relationship between (A) `mean_elevation` and `temp_90` by stream and (B) `mean_elevation` and `yday` (spawn date) by stream and year. Solid lines are linear fits."}
...
compare_aic(m.ele.1, m.ele.2, m.ele.3, m.ele.4, m.ele.5, m.ele.6, m.ele.7) |> 
  kable(caption = "AIC comparison of linear mixed models relating spawn date (`yday`) to `mean_elevation`.")
...
compare_performance(m.ele.1, m.ele.2, m.ele.3, m.ele.4, m.ele.5, m.ele.6, m.ele.7, rank = TRUE) |> 
  kable(caption = "Model performance of linear mixed models relating spawn date (`yday`) to `mean_elevation`.")
```
...
# check_model(m.ele.5, check="vif")
# check_model(m.ele.6, check="vif")
```

```{r elev-pred, fig.width=7, fig.height=4, fig.cap = "Estimated relationship between spawn date and mean elevation (m.ele.5)."}
vizdata <- get_datagrid(df_mod,
  by = c("mean_elevation", "stream", "year"), range = "grid",
  length = 100,
  preserve_range = TRUE
)

# drop sulfr and big from 2005
vizdata <- vizdata |>
  filter(!(stream == "Sulphur" & year == 2005)) |>
  filter(!(stream == "Big" & year == 2005))

# get min and max elevations for other 2005 stream
mins <- df_mod |> 
  group_by(year, stream) |> 
  summarise(min = min(mean_elevation), max = max(mean_elevation))

# filter out elevations outside of range for each stream in 2005
vizdata <- vizdata |>
  left_join(mins, by = c("stream", "year")) |>
  filter(mean_elevation >= min & mean_elevation <= max) |>
  select(-min, -max)

vizdata$Predicted_yday <- get_predicted(m.ele.5, vizdata)

ggplot(df_mod, aes(x = mean_elevation, y = yday, color = stream)) +
  geom_point(size = 0.2) +
  facet_rep_wrap(~year) + 
  geom_line(data = vizdata, aes(y = Predicted_yday), linewidth = 1) +
  theme_minimal()


# tmp <- estimate_relation(m.ele.5, preserve_range = TRUE, length = 500)
# tmp
# plot(tmp, show_data = TRUE)
```
...
Here, `COMID` will be included in all modelas as a random (intercept) effect to account for repeated measures on `COMID`s. `Stream` will be treated as a fixed effect to compare average effects across streams, and `year` as a fixed effect to compare average effects across years. The later maybe be better included as a random effect, as it is not a treatment effect, but rather a random sample of years, but we only have 4 years of data, so we will treat it as a fixed effect for now.

### Model comparison

We used linear mixed-effects models to evaluate environmental predictors of Chinook salmon spawn timing, with redd observation day-of-year (`yday`) as the response variable. Each model included a random intercept for `COMID` to account for repeated measures across stream reaches. Based on exploratory analysis and biological expectations of nonlinear thermal responses, all models containing temperature included both linear and quadratic terms for the 90-day average temperature prior to spawning (`temp_90`). We fit 31 additive models representing all combinations of four fixed effects: `stream`, `year`, `slope`, and `mean_elevation`, with temperature (including its quadratic form) included in a subset of models. Model comparison was performed using AIC, and model fit was evaluated using marginal and conditional R², RMSE, and intraclass correlation coefficients (ICC). All models were fit using maximum likelihood (REML = FALSE) for consistent comparison of fixed effects.

```{r lm, echo=FALSE, eval=FALSE}
# All combinations of 1–5 fixed effects: 31 total
m1 <- lmer(yday ~ temp_90 + (1|COMID), data = df_mod, REML = FALSE)
...
m6  <- lmer(yday ~ temp_90 + stream + (1|COMID), data = df_mod, REML = FALSE)
m7  <- lmer(yday ~ temp_90 + year + (1|COMID), data = df_mod, REML = FALSE)
m8  <- lmer(yday ~ temp_90 + mean_elevation + (1|COMID), data = df_mod, REML = FALSE)
m9  <- lmer(yday ~ temp_90 + slope + (1|COMID), data = df_mod, REML = FALSE)
...
m16 <- lmer(yday ~ temp_90 + stream + year + (1|COMID), data = df_mod, REML = FALSE)
m17 <- lmer(yday ~ temp_90 + stream + mean_elevation + (1|COMID), data = df_mod, REML = FALSE)
m18 <- lmer(yday ~ temp_90 + stream + slope + (1|COMID), data = df_mod, REML = FALSE)
m19 <- lmer(yday ~ temp_90 + year + mean_elevation + (1|COMID), data = df_mod, REML = FALSE)
m20 <- lmer(yday ~ temp_90 + year + slope + (1|COMID), data = df_mod, REML = FALSE)
m21 <- lmer(yday ~ temp_90 + mean_elevation + slope + (1|COMID), data = df_mod, REML = FALSE)
...
m26 <- lmer(yday ~ temp_90 + stream + year + mean_elevation + (1|COMID), data = df_mod, REML = FALSE)
m27 <- lmer(yday ~ temp_90 + stream + year + slope + (1|COMID), data = df_mod, REML = FALSE)
m28 <- lmer(yday ~ temp_90 + stream + mean_elevation + slope + (1|COMID), data = df_mod, REML = FALSE)
m29 <- lmer(yday ~ temp_90 + year + mean_elevation + slope + (1|COMID), data = df_mod, REML = FALSE)
m30 <- lmer(yday ~ stream + year + mean_elevation + slope + (1|COMID), data = df_mod, REML = FALSE)

m31 <- lmer(yday ~ temp_90 + stream + year + mean_elevation + slope + (1|COMID), data = df_mod, REML = FALSE)
```
...
Overall, the best-fitting model was `r aic_tab[1,]$Model`, though its improvement in AIC and performance over `r aic_tab[2,]$Model` was modest (Table \@ref(tab:aic-tab) and \@ref(tab:perf-lm)). 

Model m16 had nearly identical predictive accuracy (RMSE = 2.02) and conditional R² (0.980) compared to m26, despite a slightly lower marginal R² (0.729 vs. 0.785). The 117-point AIC difference between m16 and m26 likely reflects subtle improvements in likelihood fit due to topographic covariates.
...
However, visual inspection of predicted vs. observed effects (Figure \@ref(fig:lm-plot) B)) revealed that the modeled elevation effect was inconsistent with ecological expectations. Specifically, m26 predicted later spawning at higher elevations, contrary to the observed negative relationships between elevation, temperature, and spawn timing in several streams (Figure \@ref(fig:elev-plots)).
...
```{r lm-plot, fig.width=8, fig.height=3, fig.cap="Predicted relationship (red line) between spawn date (`yday`) and (A) `temp_90`, (B) `mean_elevation`."}
a <- plot_model(
...
This apparent discrepancy is likely due to collinearity between elevation and stream identity. Variance inflation factors (VIFs) from m26 indicated moderate collinearity between elevation and stream (VIF = 6.50 and 6.48, respectively). Elevation was strongly confounded with stream identity in our dataset (Figure \@ref(fig:elev-plots)), with high-elevation streams showing considerable overlap in spawn timing and temperature.
...
We evaluated models both with and without topographic predictors (elevation and slope). While models including these variables had stronger likelihood-based support (ΔAIC > 100), they produced counterintuitive effect estimates likely driven by spatial confounding with stream identity (see Results). We therefore focused interpretation on the top-performing model excluding topography (m16), which balanced predictive accuracy and parsimony.

**For Resutls**

While model m26 received the strongest AIC support among the candidate set, we ultimately selected model m16, which excluded elevation and slope, as the most interpretable and parsimonious model. Model m16 had nearly identical predictive accuracy (RMSE = 2.02) and conditional R² (0.980) compared to m26, despite a slightly lower marginal R² (0.729 vs. 0.785). The 117-point AIC difference between m16 and m26 likely reflects subtle improvements in likelihood fit due to topographic covariates. However, visual inspection of predicted vs. observed effects (Fig. X) revealed that the modeled elevation effect was inconsistent with ecological expectations. Specifically, m26 predicted later spawning at higher elevations, contrary to the observed negative relationships between elevation, temperature, and spawn timing in several streams (Fig. Y).

This apparent discrepancy is likely due to collinearity between elevation and stream identity. Variance inflation factors (VIFs) from m26 indicated moderate collinearity between elevation and stream (GVIF^(1/(2×Df)) = 2.54 and 1.14, respectively). Elevation was strongly confounded with stream identity in our dataset (Fig. Y), with high-elevation streams showing considerable overlap in spawn timing and temperature. As a result, the elevation effect in m26 may reflect stream-level differences rather than a general topographic control. Although it's possible that the modeled effect reflects a multivariate interaction (e.g., where temperature, elevation, and stream identity are partially redundant), we chose m16 for its more direct and interpretable structure, especially given the conceptual and empirical clarity of the relationships among temperature, stream, and year.
