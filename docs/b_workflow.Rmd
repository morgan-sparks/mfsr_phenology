...
aic_tab |> 
  ...
  select(Model, df, AIC, delta, R2_marginal, R2_conditional, RMSE, ICC) |> 
...
perf_tab |>
  kable(
    caption = "Model performance for for `m16` vs. `m16_rs`."
  )
```
...
```{r m16-rs-noquad-perf}
perf_tab |>
  kable(
    caption = "Model performance for for `m16_rs` vs. `m16_rs_noquad`."
  )
```
...
perf_tab |>
  kable(
    caption = "Model performance for `m201` vs. `m202`."
  )
```
...
```{r diagnostics, fig.width=8, fig.height=9, fig.cap="Residual diagnostics for `mod_final`."}
check_model(
  mod_final,
