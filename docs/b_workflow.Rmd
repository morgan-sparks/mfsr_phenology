...
![Map of the Middle Fork Salmon River (MFSR) study area showing redd locations used in the analysis (2002-2005) and stream reaches.](images/MFsalmonRedds_May2.jpg){width="50%"}
...
-   strong curvature captured overall: most stream × year panels show a clean, concave-up trend, matching the expected acceleration of spawning with rising temps.
-   confidence ribbons are narrow in data-dense cells and wider where observations are sparse — great to see.
...
-   Big and Camas
    ...
    -   Or abrupt temperature transitions across years or geomorphic controls
-   Loon and Marsh
    -   Patterns are tighter but still show micro-clustering → likely subtle COMID-level shifts.

Interpretation

-   The quadratic model captures macro-scale thermal responses well, but within-stream variation suggests that local (COMID-scale) differences are meaningful.
...
-   evidence that the quadratic term explains real signal above and beyond COMID-level differences
...
-   The combined model (m9_re) is strongly supported: it reflects both spatial variation and nonlinear phenological drivers
...
ggplot(newdata_comid, aes(x = temp_90, y = pred, group = COMID, color = COMID)) +
  geom_line(alpha = 0.6) +
  facet_wrap(~stream) +
  labs(title = "Predicted Spawn Timing by COMID (Simplified Model)",
...
ggplot(ranef_comid, aes(x = reorder(COMID, `(Intercept)`), y = `(Intercept)`)) +
  geom_point() +
...
-   Parallel lines = the model assumes that all COMIDs in a stream respond similarly to temperature, but start at different baselines
...
Temporal compression in lower reaches? Dan's field observation: fish in warmer, lower reaches spawn later and over a shorter window.
...
Should we add COMID slopes? The next logical step is to consider random slopes. This lets each COMID have its own slope for temp_90, capturing different rates of response to temperature.
...
m2 <- lm(yday ~ poly(flow_90, 2), data = model_data)
...
```{r flow_90_preds_year, fig.height=6}
new_data <- expand.grid(
...
new_data |> 
  ggplot(aes(x = flow_90, y = pred)) + 
  geom_ribbon(aes(ymin = lower, ymax = upper), fill = "grey80", alpha = 0.4) + 
  geom_line(color = "steelblue") + 
  facet_rep_grid(~year) + 
  geom_point(data = model_data, aes(flow_90, yday)) +
  theme_bw()
```

```{r flow_90_preds_all, fig.height=12}
new_data <- expand.grid(
  flow_90 = seq(min(model_data$flow_90), max(model_data$flow_90), length.out = 100), 
  stream = unique(model_data$stream), 
  year = unique(model_data$year)
...
Ecological Interpretation - Spawn timing is earlier in years or settings with higher pre-spawn flow" — this makes sense, especially if: - Higher flows coincide with spring snowmelt → fish advance spawning - Lower flows delay environmental cues
...
**Recommendation**: Drop flow_90 from model (or at most, keep it as a year-level covariate if we summarize it to a single annual value for all observations)
