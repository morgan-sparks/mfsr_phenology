...
```{r ranefs-comid-temp90, fig.width=7, fig.height=5, fig.cap = "Predicted spawn timing by 90-day pre-spawn temperature and COMID. Each line represents the predicted spawn timing for a specific COMID, colored by stream. The black line and shaded ribbon represents the 95% confidence interval for the population-level predictions."}

# Those sub-July predictions are a problem. The spline is being extrapolated beyond the observed temperature range for some COMIDs — when the random slope pulls a reach's intercept very negative AND the spline is evaluated at cold temperatures outside the data range, you get biologically impossible predictions. The quadratic had the same potential issue but its shape was more constrained at the cold end. The fix is straightforward — clip predictions to the observed temperature range per COMID rather than the global range. In your Figure 7C code, change the prediction grid
