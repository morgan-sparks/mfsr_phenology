...
Among temperature variables, `temp_90` clearly shows the strongest relationship with spawn date (Figure \@ref(fig:plot-covars)), and is highly colinear with `temp_30` and `temp_60` (Figure \@ref(fig:colin)).
...
```{r m16-re-quad-rs-2-plot, fig.width=8, fig.height=3, out.width="90%", fig.cap = "Model predictions (red lines) for `m16\\_rs` and `m16\\_rs\\_noquad`."}
...
```{r interact-plots, fig.width=8, fig.height=3, out.width="90%", fig.cap="Predicted spawn timing."}
...
DHARMa diagnostics indicated adequate overall fit but revealed a systematic trend in quantile residuals across the temperature gradient, with residuals shifted high at cold sites and low at warm sites. This pattern suggested the quadratic term did not fully capture the nonlinear temperature–spawn timing relationship, motivating evaluation of a more flexible functional form. 

```{r out.width="80%"}
# Simulate residuals from current final model (conditional, includes random effects)
sim_resid <- DHARMa::simulateResiduals(m16_rs, n = 500)

# Overall QQ and residuals vs. predicted
plot(sim_resid, quantreg = TRUE)

# The KS test (p≈0) and outlier test (p=1e-05) are flagged significant, and the right panel shows those wavy red quantile curves — a real pattern. However, with n=3,016, DHARMa's KS test will flag very small deviations as significant. The QQ plot itself actually looks quite good visually — points track the diagonal closely. 

# The residuals vs. predicted pattern (the S-curve in the quantile lines) is more worth attending to. It suggests mild non-uniformity across the prediction range, likely reflecting the distributional asymmetry in spawn timing at the tails (early and late spawners are harder to predict precisely).
...
# There's a clear declining trend in the quantile smoother across the temp_90 range — residuals are shifted high at low temperatures and low at high temperatures. This suggests the model is slightly over-predicting spawn timing at cold sites and under-predicting at warm sites, even after the quadratic term. It's not catastrophic, but it does suggest the nonlinear temperature relationship may not be fully captured by the quadratic alone, or that there's an interaction the model isn't accounting for.

# Both are flagged significant (red), and you can see the heterogeneity — Elk and Marsh in particular have compressed residual distributions (boxes sitting well below 0.5), while Big and Camas are shifted high. The year plot shows 2005 shifted notably lower. This pattern tells you the model's fixed effects for stream and year are not fully absorbing those group-level differences — there's systematic residual structure by group.
```
...
```{r mod-compare-plots, out.width="90%", fig.cap="Prediction comparion plots."}
...
bind_rows(pred_quad, pred_s3, pred_s4) |>
  ggplot() +
  geom_point(
    data = model_data, aes(x = temp_90, y = yday), alpha = 0.15, size = 0.8) +
  geom_ribbon(
    aes(x = temp_raw, ymin = conf.low, ymax = conf.high, fill = model),
    alpha = 0.2) +
  geom_line(aes(x = temp_raw, y = predicted, color = model), linewidth = 1.2) +
  scale_color_manual(
    values = c(
      "Quadratic" = "steelblue", 
      "Spline (df=3)" = "firebrick",
      "Spline (df=4)" = "darkgreen")) +
  scale_fill_manual(
    values  = c(
      "Quadratic" = "steelblue",
      "Spline (df=3)" = "firebrick", 
      "Spline (df=4)" = "darkgreen")) +
  labs(x = "90-day mean pre-spawn temperature (°C)",
       y = "Predicted spawn day of year", color = "", fill = "") +
  theme_bw()

```

```{r out.width="90%"}
# DHARMa residuals vs temp_90 for spline df=3 vs quadratic
...
```{r fin-mod, echo=TRUE}
# Redefine mod_final as spline df=3, refit with REML for interpretation
mod_final <- lmer(
  yday ~ ns(temp_90, df = 3) + stream + year + (1 + temp_90 | COMID),
  data = df_mod, REML = TRUE
)
...
```{r sim-resid, out.width="80%"}
# calcualte simulated residuals - conditional on ranefs (omit re.form = NULL)
...
```{r fin-perf}
perf_fin <- model_performance(mod_final, metrics = "common")
perf_fin |>
  kable(
...
Parameter estimates (Table \@ref(tab:fin-param)) indicate a significant nonlinear effect of temperature on spawn timing; spawn timing advances with increasing temperature, but levels off at high temperatures.
...
The random effects structure reveals substantial variation across COMIDs. The standard deviation for random intercepts (√τ₀₀ = `r params$Coefficient[[15]]`) and random slopes for `temp_90` (√τ₁₁ = `r params$Coefficient[[16]]`) indicates strong spatial heterogeneity in both baseline timing and temperature sensitivity (Table \@ref(tab:fin-param)). The intraclass correlation (ICC) was high (`r perf_fin$ICC`), reflecting strong grouping structure at the COMID level (Table \@ref(tab:fin-perf)).

Model fit was excellent: the marginal R² (variance explained by fixed effects) was `r perf_fin$R2_marginal`, and the conditional R² (fixed + random effects) was `r perf_fin$R2_conditional` (Table \@ref(tab:fin-perf)). This suggests that the majority of explanatory power is derived from spatially varying thermal responses captured by the random slopes, with additional structure provided by the fixed effects.
...
# --- Observed temp range per stream x year (for clipping) ---
obs_range_sy <- df_mod |>
  group_by(stream, year) |>
  summarise(
...
# --- Predictions: marginal/population-level, varying stream and year ---
# predict_response with multiple terms gives you the interaction grid
me_fig5 <- predict_response(
  mod_final,
  terms = c("temp_90 [all]", "stream", "year"),
  type = "fixed"   # population-level, no random effects
)
...
me_fig5 <- me_fig5 |>
  ...
  mutate(
    x_raw = x * sd_temp + mean_temp,
    year  = as.character(year)
  )

# Clip to observed stream x year temperature range
me_fig5_filtered <- me_fig5 |>
  left_join(obs_range_sy, by = c("stream", "year")) |>
  filter(x_raw >= min_temp & x_raw <= max_temp)

# --- Raw data back-transform (if not already done) ---
df_mod_plot <- df_mod |>
  mutate(temp_90_raw = temp_90 * sd_temp + mean_temp)
...
df_mod_plot |>
  ...
  geom_ribbon(
    data = me_fig5_filtered,
    aes(
      x    = x_raw,
      ymin = fixed_date + conf.low,
      ymax = fixed_date + conf.high,
      fill = stream
    ),
    alpha = 0.15, color = NA, 
    inherit.aes = FALSE  # prevent conflict with aes from base ggplot
  ) +
  geom_line(
    data = me_fig5_filtered,
...
Random intercepts and slopes varied considerably among COMIDs, reflecting spatial heterogeneity in both average spawn timing and thermal sensitivity (Figure \@ref(fig:ranefs-plot)A). We found substantial spread in intercepts, indicating variability in baseline spawn timing among reaches. Random slopes for temp_90 likewise varied meaningfully, showing that temperature–spawn timing relationships were not constant across space. 

<!-- Sites with earlier average spawn timing (lower intercepts) generally exhibited stronger positive responses to temperature (higher slopes), whereas later-spawning sites tended to present weaker temperature effects—a pattern evident in the weak negative correlation between intercepts and slopes (r = -0.2; Figure \@ref(fig:ranefs-plot)B). These findings highlight spatial heterogeneity in phenological flexibility, with potential implications for how different Middle Fork salmon populations may respond to ongoing climate change. -->

Random intercepts and slopes were strongly negatively correlated (r = -0.83, model-estimated from the variance-covariance matrix of random effects), indicating that stream reaches with earlier baseline spawn timing exhibited substantially greater thermal sensitivity. This pattern was consistent across most data-rich streams (Bear Valley, Big, Camas, Loon), though streams with few COMIDs and compressed intercept ranges (Marsh, Sulphur, Beaver) provided insufficient within-stream variation to evaluate the relationship reliably. The correlation is considerably stronger under the natural spline formulation than under the quadratic model (r = -0.2), likely reflecting the spline's greater flexibility in capturing steep thermal responses at cold temperatures characteristic of early-spawning reaches. While this correlation may be partially amplified by the spline's functional form, its consistency across data-rich streams suggests a real biological signal: reaches that spawn earliest are most responsive to temperature variation, potentially indicating greater phenological plasticity or stronger dependence on thermal cues among high-elevation fish.

```{r ranefs-plot, fig.width=3, fig.height=2,  results='hide', fig.cap = "Correlation between random intercepts and slopes for 90-day temperature across COMIDs. Points represent best linear unbiased predictions (BLUPs) from the final model."}
...
<!-- Figure 7 further illustrates this variability across scales, summarizing the distribution of random intercepts and slopes by stream (panel A) and the corresponding predicted temperature–spawn timing relationships for individual reaches (panel B). COMID-level variation in intercepts and slopes reveals substantial spatial heterogeneity in both average timing and thermal sensitivity, with some streams showing greater within-stream variability than others. While the overall relationship between 90-day pre-spawn temperature and spawn timing is positive and nonlinear, the strength and shape of this response vary among reaches, likely reflecting fine-scale differences in hydrology, geomorphology, or biological composition. Despite this heterogeneity, the consistency of the population-level trend underscores the biological relevance of temperature as a key cue structuring spawn timing in Middle Fork Chinook Salmon populations. -->
...
```{r ranefs-bar, eval=FALSE, fig.width=8, fig.height=6, fig.cap = "Random intercepts and slopes for 90-day pre-spawn temperature by stream. Each bar represents the best linear unbiased prediction (BLUP) for a COMID's random intercept (average spawn timing) or slope (thermal sensitivity). Bars are colored by stream."}
...
```{r ranefs-comid-temp90, results='hide', fig.width=7, fig.height=5}
...
# me2 <- predict_response(mod_final, terms = c("temp_90 [all]"), interval = "prediction")
# # plot(me2, show_data = TRUE, show_ci = TRUE)
...
# save plot
path <- here::here("plots", "preds_ranef")
ggsave(
  glue::glue("{path}.pdf"),
  plot = last_plot(),
  device = cairo_pdf,
