...
# Filter to observed temp range per COMID
obs_range <- model_data |> 
  group_by(COMID) |> 
  summarise(
    min_temp = min(temp_90), 
    max_temp = max(temp_90), 
    .groups = "drop"
  )
...
p.re.preds <- me_filtered |> 
  ...
  geom_line(
    ...
    linewidth = 2) +
  scale_color_manual(values = cols.streams) +
  # scale_color_grey(start = .5, end = .9) + 
  # scale_y_date(
  #   date_breaks = "1 month", 
  #   date_labels = "%b", 
  #   limits = as.Date(c("2001-06-01", "2001-10-28"))
  #   ) +
  guides(color = "none") +
  labs(
    x = "90-day mean temperature pre-spawn (°C)",
...
(p.re.int.box + p.re.slp.box) / p.re.preds + 
  plot_layout(guides = "collect", heights = c(.75,1)) + 
  plot_annotation(
    tag_levels = "A"
  )
