...
# Check the random effects correlation parameter from the model directly
# VarCorr(mod_final)
...
# COMID - level responses -----------
# Get COMID observed temp range
...
# Get global observed temp range ------------
...
p.re.preds <- me_filtered |>
  ...
  #   date_breaks = "1 month",
  #   date_labels = "%b",
  #   limits = as.Date(c("2001-07-01", "2001-9-15"))
  #   ) +
  guides(color = "none") +
  labs(
...
<!-- ### Elevation structure in reach-level random effects -->

<!-- We examined whether the basin-wide intercept-slope correlation was also expressed as consistent within-stream elevation gradients by plotting reach-level random intercepts and slopes against mean reach elevation within each stream (Figure \@ref(fig:re-elevation-plot)) and fitting ordinary least-squares regressions within each stream. Interpreting these relationships requires noting an important structural feature of the study streams: Bear Valley, Beaver, Marsh, and Sulphur are high-elevation systems with constrained elevation ranges (< 100m spread across sampled reaches), whereas Big Creek, Camas, Elk, and Loon span broader elevation gradients (200–500m). Streams with compressed elevation ranges may show statistically significant elevation-random effect relationships simply because their reaches share similar thermal environments and life history characteristics associated with their uniformly high positions in the watershed — not because elevation per se is driving within-stream variation in timing or thermal sensitivity. Results from those streams should therefore be interpreted cautiously. We also note that these regressions are performed on model-derived quantities (BLUPs) rather than raw observations and do not propagate uncertainty from the mixed model; they are best treated as exploratory rather than confirmatory. -->

<!-- With those caveats in mind, significant positive relationships between mean reach elevation and random intercepts — indicating that higher-elevation reaches spawn later relative to their stream mean — were detected in seven of eight streams. Significant negative relationships between elevation and random slopes — indicating that higher-elevation reaches show stronger negative thermal responsiveness — were detected in four streams. The most interpretable results come from Big Creek and Camas Creek, which combine wide elevation ranges with large reach sample sizes (n = 21 and 16, respectively) and strong, significant relationships in both models. These two streams provide the clearest within-stream evidence that the basin-wide intercept-slope pattern reflects a real ecological gradient operating at fine spatial scales rather than an artifact of pooling thermally distinct streams. -->


```{r re-elevation-plot, eval=FALSE, echo=FALSE, fig.width=7, fig.height=9, fig.cap="Reach-level random intercepts (A) and random slopes (B) from the final linear mixed-effects model plotted against mean reach elevation, faceted by stream. Random intercepts represent reach-specific deviations in baseline spawn timing from the population mean (positive values = later spawning relative to stream mean; negative values = earlier). Random slopes represent reach-specific deviations in thermal responsiveness from the population mean (positive values = spawn timing advances more steeply with increasing antecedent temperature; negative values = less steeply or declines). Dashed horizontal lines indicate zero deviation. Fitted lines are ordinary least-squares regressions within each stream. Note that Bear Valley, Beaver, Marsh, and Sulphur are high-elevation systems with constrained elevation ranges across sampled reaches (< 100m), limiting the interpretability of within-stream elevation gradients in those streams. All plots share a common x-axis scale to preserve the spatial context of each stream's elevation range within the broader basin.", out.width="90%", dpi=300}

# Shared theme
theme_re <- theme_bw(base_size = 11) +
  theme(
...
p_int <- random_wide |>
  ggplot(aes(mean_elevation, Intercept, color = stream)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey60", linewidth = 0.4) +
  geom_point(size = 2.2, alpha = 0.85) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 0.7, alpha = 0.6) +
  facet_wrap(~stream) +
  scale_color_manual(values = cols.streams) + 
  labs(
    x = "Mean reach elevation (m)",
    y = "Random intercept\n(baseline spawn timing deviation, days)"
  ) +
  theme_re

p_slp <- random_wide |>
  ggplot(aes(mean_elevation, Slope, color = stream)) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "grey60", linewidth = 0.4) +
  geom_point(size = 2.2, alpha = 0.85) +
  geom_smooth(method = "lm", se = FALSE, linewidth = 0.7, alpha = 0.6) +
  facet_wrap(~stream) +
  ...
  labs(
    x = "Mean reach elevation (m)",
    y = "Random slope\n(thermal responsiveness deviation)"
  ) +
  theme_re

# Stack vertically with patchwork, add panel labels
p_int / p_slp +
  plot_annotation(tag_levels = "A")

```

```{r re-elev-models, echo=FALSE, eval=FALSE}
# Run lm within each stream for both intercept and slope
re_models <- random_wide |>
  group_by(stream) |>
  nest() |>
  mutate(
    int_mod = map(data, ~lm(Intercept ~ mean_elevation, data = .x)),
    slp_mod = map(data, ~lm(Slope ~ mean_elevation, data = .x)),
    int_tidy = map(int_mod, tidy),
    slp_tidy = map(slp_mod, tidy),
    int_glance = map(int_mod, glance),
    slp_glance = map(slp_mod, glance)
  )

# Extract elevation term p-values and R² for intercept models
# re_models |>
#   unnest(int_tidy) |>
#   filter(term == "mean_elevation") |>
#   select(stream, estimate, std.error, p.value) |>
#   left_join(
#     re_models |> unnest(int_glance) |> select(stream, r.squared, nobs = nobs),
#     by = "stream"
#   )

# Same for slope models
# re_models |>
#   unnest(slp_tidy) |>
#   filter(term == "mean_elevation") |>
#   select(stream, estimate, std.error, p.value) |>
#   left_join(
#     re_models |> unnest(slp_glance) |> select(stream, r.squared, nobs = nobs),
#     by = "stream"
#   )
...
re_table <- re_models |>
  ...
  left_join(
    re_models |>
      ...
      left_join(
        re_models |>
          unnest(int_glance) |>
          select(stream, int_r2 = r.squared, nobs),
        by = "stream"
      ),
    by = "stream"
  ) |>
  ...
  left_join(
    re_models |>
      ...
      left_join(
        re_models |>
          unnest(slp_glance) |>
          select(stream, slp_r2 = r.squared),
        by = "stream"
      ),
    by = "stream"
  ) |>
  select(
    stream, nobs,
    int_estimate, int_se, int_r2, int_p,
    slp_estimate, slp_se, slp_r2, slp_p
  ) |>
  arrange(stream) |>
  # Flag constrained elevation streams
...
```{r re-elevation-table, results='asis', echo=FALSE, eval=FALSE}
re_table |>
  kable(
    format = "latex",  # swap to "html" if knitting to HTML
    booktabs = TRUE,
    digits = c(0, 0, 3, 3, 2, 3, 3, 3, 2, 3),
    col.names = c(
      "Stream", "n",
      "β", "SE", "R²", "p",
      "β", "SE", "R²", "p"
    ),
    caption = "Ordinary least-squares regression results 
    for reach-level random intercepts and slopes regressed on mean 
    reach elevation within each stream. β = elevation slope coefficient 
    (days m⁻¹ for intercept models; thermal responsiveness units m⁻¹ 
    for slope models). n = number of reaches. † High-elevation 
    systems with constrained elevation ranges (< 100m); 
  ...
  footnote(
    symbol = "High-elevation system with constrained elevation range; interpret cautiously.",
