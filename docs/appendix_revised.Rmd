...
```{r plot-temp-regime, fig.width=8, fig.height=6, out.width="80%", fig.cap="Modeled thermal regimes (2001-2005) for MFSR tributaries. Black line are means, Red ribbon are 40 to 60th percentiles, Grey ribbon are full range."}
...
```{r plot-flow, fig.width=8, fig.height=6, out.width="80%", fig.cap = "Inter-annual variability in daily discharge (cfs) at MF Lodge USGS Gage 13309220."}
p.flow.1 <- df_flows |> 
...
```{r colin, cache=TRUE, fig.width=9, fig.height=8, out.width="80%", fig.cap="Pairwise correlations between continuous covariates."}
...
compare_aic(m.t90.1, m.t90.2, m.t90.3) |> 
  ...
  kable_styling(latex_options = c("hold_position", "HOLD_position"))
```
...
```{r flow-vif, fig.width=6, fig.height=3, fig.cap="Variance inflation factors for model m.flow.7"}
...
```{r mod-compare-plots, out.width="90%", fig.cap="Prediction comparison plots."}
...
```{r dharma-spline-compare, out.width="90%"}
...
To assess whether fixed-effect interactions provide additional explanatory power beyond what is captured by random slopes, we tested two models: one with a `ns(temp_90, df=3) × stream` interaction (`m201`) and one with a `ns(temp_90, df=3) × year` interaction (`m202`). Both were compared to the spline baseline (`mod_spline3`).

```{r interact-mods, echo=TRUE}
m201 <- lmer(yday ~ ns(temp_90, df = 3) * stream + year + (1 + temp_90 | COMID), data = df_mod, REML = FALSE)
m202 <- lmer(yday ~ ns(temp_90, df = 3) * year + stream + (1 + temp_90 | COMID), data = df_mod, REML = FALSE)
```
...
```{r interact-plots, fig.width=10, fig.height=4, out.width="95%", fig.cap="Predicted spawn timing for interaction models."}
...
```{r diagnostics, fig.width=8, fig.height=9, out.width="80%", fig.cap="Residual diagnostics for mod\\_final."}
...
```{r sim-resid1, out.width="80%", fig.cap="DHARMa residuals 1."}
...
```{r sim-resid2, out.width="80%", fig.cap="DHARMa residuals 2."}
par(mfrow = c(3, 1))
plotResiduals(sim_resid, form = df_mod$temp_90, main = "vs. temp_90")
plotResiduals(sim_resid, form = df_mod$stream,  main = "vs. stream")
plotResiduals(sim_resid, form = df_mod$year,    main = "vs. year")
...
```{r make-tables}
source(here::here("docs", "make_tables.R"))
...
```{r ranefs-box, fig.width=8, fig.height=3, fig.cap = "Boxplots of random intercepts (A) and slopes (B) for 90-day pre-spawn temperature by stream. Each box represents the distribution of BLUPs for COMID-level intercepts or slopes, colored by stream."}
