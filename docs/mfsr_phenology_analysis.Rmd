...
# count of observations by stream
spawn_data_cleaned |> 
  count(stream, name = "n_redds") |> 
  arrange(desc(n_redds)) |> 
  knitr::kable(
    caption = "Count of Chinook salmon redds by stream (unfiltered data)."
  )
spawn_data |> 
  count(stream, name = "n_redds") |> 
  arrange(desc(n_redds)) |> 
  knitr::kable(
    caption = "Count of Chinook salmon redds by stream (filtered data)."
  )
```
...
Based on AIC, the best-fitting model was `m26`, though its improvement in AIC and performance over `m31`, the full model which includes `slope`, was modest (Table \@ref(tab:aic-tab) and \@ref(tab:perf-lm)). Suggests little benefit to including `slope` as a predictor.
...
Variance inflation factors (VIFs) from m26 indicated moderate collinearity between elevation and stream (VIF = 6.50 and 6.48, respectively). Elevation was strongly confounded with stream identity in our dataset (Figure \@ref(fig:elev-plots)), with high-elevation streams showing considerable overlap in spawn timing and temperature.
...
lm_tab |>
  kable(
    caption = "Model selection for `m16` vs. `m16_rs`."
  )
```
...
tab_model(
  ...
  title = "(\\#tab:slope-tab) Parameter estimates for `m16` and `m16_rs`.",
  CSS = list(
...
```{r plot, eval=FALSE, fig.width=8, fig.height=3, fig.cap = "Model predictions (red lines) for `m16` and `m16_rs`."}
...
lm_tab |>
  kable(
    caption = "Model selection for `m16_rs` vs. `m16_rs_noquad`."
  )
```

```{r m16-re-quad-rs-2-plot, fig.width=8, fig.height=3, fig.cap = "Model predictions (red lines) for `m16_rs` and `m16_rs_noquad`."}
...
```{r diagnostics, cache=FALSE, fig.width=8, fig.height=9, fig.cap="Residual diagnostics for `mod_final`."}
check_model(
  mod_final,
...
# Plot cor colored by stream
# random_wide |> 
#   # filter(!stream %in% c("Bear Valley","Marsh","Elk","Beaver")) |>
#   ggplot(aes(x = Intercept, y = Slope, color = stream)) +
#   # geom_point(aes(size = mean_elevation)) +
#   geom_point() +
